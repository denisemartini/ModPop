...
```{r hwe_write}
write.table(out_of_hwe[,1:2], file = "loci_outofHWE.txt", sep = "_", row.names = FALSE, col.names = FALSE, quote = FALSE)
```
...
length(dups) # number of distinct locu that are in LD with more than one other locus
```
...
n_distinct(discard) # number of loci that will need to be discarded
n_distinct(pos) == (n_distinct(keep)+n_distinct(discard))  # sanity check
```
...
```{r ld_write}
write.table(discard, file = "loci_inLD.txt", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
write.table(keep, file = "loci_inLD_kept.txt", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
```
